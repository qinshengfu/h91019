 ...
 */
@Controller
@RequestMapping(value="/jfproduct")
public class JFProductController extends BaseController {
	
	String menuUrl = "jfproduct/list.do"; //菜单地址(权限用)
	...
	public ModelAndView save() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"新增JFProduct");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "add")){return null;} //校验权限
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		pd.put("STATE", "1");
		pd.put("JFPRODUCT_ID", this.get32UUID());	//主键
		pd.put("TIME", Tools.date2Str(new Date()));	//时间
		jfproductService.save(pd);
	...
	public void delete(PrintWriter out) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"删除JFProduct");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "del")){return;} //校验权限
		PageData pd = new PageData();
		pd = this.getPageData();
		jfproductService.delete(pd);
		out.write("success");
		out.close();
	}
	...
	public ModelAndView edit() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"修改JFProduct");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "edit")){return null;} //校验权限
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		jfproductService.edit(pd);
		mv.addObject("msg","success");
		mv.setViewName("save_result");
		return mv;
	}
	...
	public ModelAndView list(Page page) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"列表JFProduct");
		...
		page.setPd(pd);
		List<PageData>	varList = jfproductService.list(page);	//列出JFProduct列表
		mv.setViewName("setup/jfproduct/jfproduct_list");
	...
	public ModelAndView goAdd()throws Exception{
		...
		mv.setViewName("setup/jfproduct/jfproduct_edit");
	...
	public ModelAndView goEdit()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		pd = jfproductService.findById(pd);	//根据ID读取
	...
	public Object deleteAll() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"批量删除JFProduct");
		...
		if(null != DATA_IDS && !"".equals(DATA_IDS)){
			...
			jfproductService.deleteAll(ArrayDATA_IDS);
			pd.put("msg", "ok");
		}else{
			pd.put("msg", "no");
		}
	...
	public ModelAndView exportExcel() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"导出JFProduct到excel");
		...
		List<PageData> varOList = jfproductService.listAll(pd);
